...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZsombeThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the SBE and zSBE money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 25184, 37490));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SBE specific functionality (Masternodes, Zerocoin, iBanX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-sombestake=<n>", strprintf(_("Enable or disable staking functionality for SBE inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zsombestake=<n>", strprintf(_("Enable or disable staking functionality for zSBE inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:25184"));
    ...
    strUsage += HelpMessageOpt("-backupzsombe=<n>", strprintf(_("Enable automatic wallet backups triggered after each zSBE minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zsombebackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSBE backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 25185, 51475));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzSBEWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zSBEWallet
        uiInterface.InitMessage(_("Syncing SBE wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZSbeBackups = GetBoolArg("-backupzsombe", true);
        pwalletMain->setZSbeAutoBackups(fEnableZSbeBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zsombeTracker->Init();
...
//    if (nAnonymizeSombeAmount < 2) nAnonymizeSombeAmount = 2;
    ...
    LogPrintf("Anonymize SBE Amount %d\n", nAnonymizeSombeAmount);
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZsombeThread(threadGroup);
    }
