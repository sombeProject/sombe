...
CzSBEWallet::CzSBEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsombe seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSBESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSBESeed_deprecated()) {
                    LogPrintf("%s: Updated zSBE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsombe seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsombe wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSBEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSBECount(nCountLastUsed);
    else if (!walletdb.ReadZSBECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSBEWallet::Lock()
{
    seedMaster = 0;
}

void CzSBEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSBEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSBE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSBEWallet::LoadMintPoolFromDB()
{
...
void CzSBEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSBEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSBEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsombeTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSBEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSBE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsombeTracker which also adds to database
    pwalletMain->zsombeTracker->Add(dMint, true);
...
void CzSBEWallet::SeedToZSBE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSBEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSBEWallet::GenerateDeterministicZSBE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSBEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSBEWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSBEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
