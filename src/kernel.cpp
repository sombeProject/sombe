...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSbeStake(spend));
    } else {
        ...
        CSbeStake* sombeInput = new CSbeStake();
        sombeInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(sombeInput);
    }
