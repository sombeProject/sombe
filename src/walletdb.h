...
public:
    ...
    bool WriteZSBESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSBESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSBESeed_deprecated(uint256& seed);
    bool EraseZSBESeed();
    bool EraseZSBESeed_deprecated();

    bool WriteZSBECount(const uint32_t& nCount);
    bool ReadZSBECount(uint32_t& nCount);
