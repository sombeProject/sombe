...
CzSBETracker::CzSBETracker(std::string strWalletFile)
{
...
CzSBETracker::~CzSBETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSBETracker::Init()
{
...
bool CzSBETracker::Archive(CMintMeta& meta)
{
...
bool CzSBETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSBETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSBETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSBETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSBETracker::GetSerialHashes()
{
...
CAmount CzSBETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSBETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSBETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSBETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSBETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSBETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSBETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSBETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSBETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSBETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSBETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSBEWallet* zSBEWallet)
{
    bool iszSBEWalletInitialized = (NULL != zSBEWallet);
    CMintMeta meta;
    ...
    if (! iszSBEWalletInitialized)
        zSBEWallet = new CzSBEWallet(strWalletFile);
    meta.isSeedCorrect = zSBEWallet->CheckSeed(dMint);
    if (! iszSBEWalletInitialized)
        delete zSBEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSBETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSBETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSBETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSBETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSBETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSBETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSBEWallet* zSBEWallet = new CzSBEWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zSBEWallet);
        }
        delete zSBEWallet;
        LogPrint("zero", "%s: added %d dzsombe from DB\n", __func__, listDeterministicDB.size());
    }
