                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    // If iBanX activated hide button 'Choose'. Show otherwise.
    ui->buttonChooseFee->setVisible(!useiBanTX);
}
...
{
    bool useiBanTX = ui->checkiBanTX->isChecked();

    QSettings settings;
    settings.setValue("bUseiBanTX", useiBanTX);
    CoinControlDialog::coinControl->useiBanTX = useiBanTX;
...
{
    ui->sliderSmartFee->setEnabled(ui->radioSmartFee->isChecked() && !ui->checkiBanTX->isChecked());
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    // if iBanX checked, display it in the label
    if (ui->checkiBanTX->isChecked())
    {
        ui->labelFeeEstimation->setText(tr("Estimated to get 6 confirmations near instantly with <b>iBanX</b>!"));
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid SBE address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
