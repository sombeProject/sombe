                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zSBE ought to be enough for anybody." - Bill Gates, 2017
    ui->zSBEpayAmount->setValidator( new QDoubleValidator(0.0, 500000000.0, 20, this) );
    ...
    ui->labelzSBESyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSBEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSBE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSBE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zSBE, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSBE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSBE in ") +
...
void PrivacyDialog::on_pushButtonSpendzSBE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSBE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zSBE, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zSBE
        sendzSBE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSBE
    sendzSBE();
}

void PrivacyDialog::on_pushButtonZSbeControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZSbeControlDialog* zSbeControl = new ZSbeControlDialog(this);
    zSbeControl->setModel(walletModel);
    zSbeControl->exec();
}

void PrivacyDialog::setZSbeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSbeSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSBE()
{
    ...
    double dAmount = ui->zSBEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSBE is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSBE</b>";
    ...
    // use mints from zSBE selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSbeControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZSbeControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSBE require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zSBE"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZSBE_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSBE require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zSBE"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSBE was spent successfully update the addressbook with the label
    ...
    // Clear zsombe selector in case it was used
    ZSbeControlDialog::setSelectedMints.clear();
    ui->labelzSbeSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSBE Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& earnings, const CAmount& masternodeEarnings, const CAmount& stakeEarnings)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zsombeTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSBE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSBE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSBE "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zSBE "));
    ui->labelzSBEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zSBE </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zSBE </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zSBE </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzSBESyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzSBE->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zSBE
        ui->pushButtonMintzSBE->setEnabled(false);
        ui->pushButtonMintzSBE->setToolTip(tr("zSBE is currently disabled due to maintenance."));

        // Spend zSBE
        ui->pushButtonSpendzSBE->setEnabled(false);
        ui->pushButtonSpendzSBE->setToolTip(tr("zSBE is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zSBE
        ui->pushButtonMintzSBE->setEnabled(true);
        ui->pushButtonMintzSBE->setToolTip(tr("PrivacyDialog", "Enter an amount of SBE to convert to zSBE", 0));

        // Spend zSBE
        ui->pushButtonSpendzSBE->setEnabled(true);
        ui->pushButtonSpendzSBE->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
