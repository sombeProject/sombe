...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sSBEPercentage, QString& szSBEPercentage)
{
    int nPrecision = 2;
    ...
    szSBEPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& earnings, const CAmount& masternodeEarnings, const CAmount& stakeEarnings)
{
    ...
    CAmount sombeAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // SBE Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = sombeAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // SBE labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, sombeAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    QString automintHelp = tr("Current percentage of zSBE.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    ...
    // SBE Available
    bool showSBEAvailable = settingShowAllBalances || sombeAvailableBalance != nTotalBalance;
    bool showWatchOnlySBEAvailable = showSBEAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ...
    ui->labelWatchAvailable->setVisible(showWatchOnlySBEAvailable && showWatchOnly);

    // SBE Pending
    bool showSBEPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlySBEPending = showSBEPending || watchUnconfBalance != 0;
    ...
    ui->labelWatchPending->setVisible(showWatchOnlySBEPending && showWatchOnly);

    // SBE Immature
    bool showSBEImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showSBEImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showSBEImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showSBEImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // SBE Locked
    bool showSBELocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlySBELocked = showSBELocked || nWatchOnlyLockedBalance != 0;
    ...
    ui->labelWatchLocked->setVisible(showWatchOnlySBELocked && showWatchOnly);
    ...
    ui->labelSBEPercent->setVisible(false);
    ui->labelzSBEPercent->setVisible(false);
