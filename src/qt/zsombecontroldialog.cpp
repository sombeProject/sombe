...
#include "ui_zsombecontroldialog.h"
...
std::set<std::string> ZSbeControlDialog::setSelectedMints;
std::set<CMintMeta> ZSbeControlDialog::setMints;

bool CZSbeControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSbeControlDialog::COLUMN_DENOMINATION || column == ZSbeControlDialog::COLUMN_VERSION || column == ZSbeControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZSbeControlDialog::ZSbeControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSbeControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSbeControlDialog::~ZSbeControlDialog()
{
    delete ui;
}

void ZSbeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSbeControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSbeControlWidgetItem* itemDenom(new CZSbeControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSbeControlWidgetItem *itemMint = new CZSbeControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zSBE seed used to mint this zSBE is not the same as currently hold in the wallet";
            else
...
void ZSbeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSbeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSbe_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSbeControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSbeControlDialog::GetSelectedMints()
{
...
void ZSbeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
