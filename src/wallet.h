...
// Possible states for zSBE send
enum ZerocoinSpendStatus {
    ZSBE_SPEND_OKAY = 0,                            // No error
    ZSBE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSBE_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSBE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSBE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSBE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSBE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSBE_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSBE_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSBE_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSBE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSBE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSBE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSBE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSBE_SPENT_USED_ZSBE = 14,                      // Coin has already been spend
    ZSBE_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSBE_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZSbeBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSBEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzsombeAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzSBETracker> zsombeTracker;
    ...
    void setZWallet(CzSBEWallet* zwallet)
    {
        zwalletMain = zwallet;
        zsombeTracker = std::unique_ptr<CzSBETracker>(new CzSBETracker(strWalletFile));
    }

    CzSBEWallet* getZWallet() { return zwalletMain; }
    ...
    void setZSbeAutoBackups(bool fEnabled)
    {
    ...
    /** zSBE reset */
    boost::signals2::signal<void()> NotifyzSBEReset;
